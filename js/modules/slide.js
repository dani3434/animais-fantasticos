...
export class Slide {
  ...
  onMove(event) {
    const eventsType = (event.type === 'mousemove') ? event.clientX : event.changedTouches[0].clientX;
    const finalPosition = this.updatePosition(eventsType);
    this.slideMove(finalPosition);
  }

  onEnd(event) {
    const movesType = (event.type === 'mouseup') ? 'mousemove' : 'touchmove';
    this.wraper.removeEventListener(movesType, this.onMove);
  ...
  changeSlide(index) {
    ...
    this.slideMove(activeSlide.position);
